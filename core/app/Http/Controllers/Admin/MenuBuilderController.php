...
class MenuBuilderController extends Controller
{
    ...
    public function megaMenuEdit(Request $request) {
        ...
        // for 'timetable' mega menu
        if ($request->type == 'timetable') {
            $data['cats'] = $lang->time_categories()->get();
            $megamenu = Megamenu::where('language_id', $lang->id)->where('type', 'timetable')->where('category', 1);
            $catStatus = 1;
        }
    ...
    public function megaMenuUpdate(Request $request) {
        ...
        if ($type == 'services') {
            ...
            if (!empty($items)) {
                ...
                $megamenu = Megamenu::where('language_id', $langid)->where('type', 'timetable')->where('category', 1)->firstOrFail();
            }
        } elseif ($type == 'blogs') {
            if (!empty($items)) {
                foreach ($items as $key => $item) {
