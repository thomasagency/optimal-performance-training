...
class LanguageController extends Controller
{
    ...
    public function delete($id)
    {
        ...
        //timetable
        $timetable = TimeTable::query()->where('lang_id', $id)->get();
        DB::transaction(function () use ($time_tables, $id) {
            foreach ($timetable as $time) {
                
               
                $images = json_decode($time->image);
                if (count($images) > 0) {
                    foreach ($images as $image) {
                        $directory = 'assets/front/img/timetable/sliders/' . $image;
                        if (file_exists($directory)) {
                            @unlink($directory);
                        }
                    }
                ...
                $time->delete();
                TimeCategory::query()->where('lang_id', $id)->delete();
            }
        });
